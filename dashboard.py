...
p2 = pn.indicators.Gauge(name="DC Inv Power", 
                         value=np.round(power.DCInverterWatts.resample(time='1h').mean().values[-1]), 
                         bounds=(0, 250), 
                         format='{value} W', 
...
p5 = BatterySOC.hvplot(grid=True, 
                       ...
                       color='lightseagreen', responsive=True).opts(active_tools=['box_zoom']) * 
    BatteryWatts.hvplot(grid=True, 
                        line_width=5, 
                        height=400, 
                        color='lightblue', 
                        responsive=True)
...
p6 = SolarWatts_Tot.hvplot(grid=True, 
                           line_width=5, 
                           height=400, 
                           title='RENEWABLES (most recent measurement from '+pd.to_datetime(power.time.data[-1]).strftime('%H:%M')+')', color='sienna', responsive=True).opts(active_tools=['box_zoom']) * 
        WindWatts.hvplot(line_width=5, 
                         height=400, 
                         color='darkkhaki', 
                         responsive=True)
